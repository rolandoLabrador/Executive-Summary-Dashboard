...
async function run() {
  ...
  let targetClaims = report.claims.filter(c => 
    c.activityDate.getFullYear() === 2026 && c.activityDate.getMonth() === 7
  );
  ...
  for (const [claimNo, csvPaid] of csvAmounts.entries()) {
    ...
    if (Math.abs(csvPaid - dbPaid) > 0.01) {
      console.log(`Discrepancy for ${claimNo}: CSV = ${csvPaid}, DB = ${dbPaid}, Diff = ${csvPaid - dbPaid}`);
